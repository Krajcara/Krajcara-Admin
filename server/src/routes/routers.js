...
router.post('/', requireRole('superadmin', 'admin'), (req, res) => {
  ...
  `).run(
    ...
    snmp_username || null, snmp_auth_protocol || 'SHA', snmp_auth_password ? snmp_auth_password : null,
    snmp_priv_protocol || 'AES', snmp_priv_password ? snmp_priv_password : null, snmp_security_level || 'authPriv'
  );
