...
function mapLicence(l) {
  return {
    ...
    licence_password: l.licence_password ? l.licence_password : null,
  };
}
...
router.post('/entra-apps', (req, res) => {
  ...
  const r = db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?)
  `).run(app_name.trim(), app_id || null, client_secret ? client_secret : null, secret_expiry || null, assigned_to || null, project || null, notes || null);
