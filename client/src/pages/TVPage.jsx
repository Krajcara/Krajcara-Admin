...
import { Wifi, Download, Upload, Activity, Server, Shield, AlertCircle, AlertTriangle, Info, CheckCircle, RefreshCw } from 'lucide-react'
        ...
        {monitors.map(m => {
          ...
          return (
            <div key={m.id} className={cn('flex items-center gap-2.5 px-3 py-2 rounded-lg',
              ...
              <span className={cn('flex-1 text-sm font-medium truncate', isDown ? 'text-red-300' : 'text-white')}>
                {m.label}
              </span>
              {m.last_latency_ms != null && (
                <span className="text-xs text-gray-500 font-mono">{m.last_latency_ms}ms</span>
              )}
              <span className={cn('text-xs font-bold px-1.5 py-0.5 rounded',
                ...
                {(m.last_status || 'unknown').toUpperCase()}
              </span>
            </div>
          )
        })}
          ...
          <div key={node.node} className="bg-gray-800/50 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-semibold text-gray-300">
                ...
                <span className="text-gray-500 text-xs ml-1">running</span>
              </span>
            </div>
...
function NetSpeedPanel({ lastSpeed }) {
  return (
    <div className="flex flex-col h-full">
      ...
      {!lastSpeed ? (
        <div className="flex items-center justify-center flex-1 text-gray-600 text-sm">No data</div>
      ) : (
        <div className="flex-1 space-y-3">
          {[
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        ...
        {unread.length > 0 && (
          <span className="text-xs bg-red-500 text-white rounded-full px-2 py-0.5 font-bold">{unread.length} new</span>
        )}
      </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5">
          {notifications.map(n => (
            <div key={n.id} className={cn('flex items-start gap-2.5 px-3 py-2.5 rounded-r-lg border-l-2',
              TYPE_BG[n.type] || TYPE_BG.info, !n.read && 'opacity-100', n.read && 'opacity-50')}>
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col" style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-gray-900 border-b border-gray-800 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-600 text-sm">— System Monitor</span>
        </div>
        <div className="flex items-center gap-4 text-sm text-gray-400">
          ...
          <span className="text-white font-mono text-base">
            {now.toLocaleDateString('en', { day: '2-digit', month: '2-digit', year: 'numeric' })}
            {' '}
            <Clock />
          </span>
        </div>
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-3">
            <RefreshCw className="w-10 h-10 text-gray-600 animate-spin mx-auto" />
      ...
      {/* Footer */}
      <div className="flex-shrink-0 text-center py-2 text-xs text-gray-700 border-t border-gray-900">
        Auto-refresh every 30 seconds · /tv
      </div>
    </div>
  )
}
