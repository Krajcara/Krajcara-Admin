...
import { RefreshCw, Shield, CheckCircle, Package, Clock, XCircle, Monitor, Server, Box } from 'lucide-react'
...
function VMDetail({ vm, onClose }) {
  ...
  useSocket({
    'patches:done': ({ vm_id }) => { if (!vm_id || vm_id === vm.vm_id) load() }
  })
  ...
  return (
    <div className="space-y-4">
      ...
      {isLxc ? (
        <AlertBox type="info">
          LXC containers are listed for overview only. Patch checking via Guest Agent exec is not supported for LXC. Use Proxmox Shell or SSH to manage updates manually.
        </AlertBox>
      ) : loading ? (
...
// ── VM card ───────────────────────────────────────────────────────────────────
function VMCard({ vm, onSelect }) {
  const isLxc       = vm.vm_type === 'lxc'
  const hasUpdates  = vm.update_count > 0
  const hasSecurity = vm.security_count > 0

  // LXC — always neutral (no alarm)
  const barColor = isLxc
    ? 'bg-gray-200 dark:bg-gray-700'
    : hasSecurity ? 'bg-orange-500'
    : hasUpdates  ? 'bg-yellow-400'
    : vm.status === 'error' ? 'bg-red-400'
    : 'bg-green-500'

  const borderClass = !isLxc && hasSecurity ? 'border-orange-300 dark:border-orange-700' : ''

  return (
    <Card className={cn('overflow-hidden cursor-pointer hover:shadow-md transition-shadow', borderClass)}
      onClick={() => onSelect(vm)}>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className={cn('w-2.5 h-2.5 rounded-full flex-shrink-0',
              isLxc ? 'bg-gray-400' : hasSecurity ? 'bg-orange-500' : hasUpdates ? 'bg-yellow-400' : vm.status === 'error' ? 'bg-red-400' : 'bg-green-500'
            )} />
            <div>
        ...
        {isLxc ? (
          <div className="flex items-center gap-1.5 text-xs text-gray-400">
            ...
            <span>Listed — no patch check</span>
          </div>
        ) : vm.status === 'error' ? (
...
// ── VM Group section ──────────────────────────────────────────────────────────
function VMGroup({ title, icon: Icon, vms, onSelect, emptyMsg }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {vms.map(vm => (
          <VMCard key={`${vm.node}-${vm.vm_id}`} vm={vm} onSelect={onSelect} />
        ))}
      </div>
    </div>
  )
...
export default function PatchManagementPage() {
  ...
  // Groups
  const linuxVMs   = vms.filter(v => v.vm_type === 'qemu' && v.os_type !== 'windows')
  const windowsVMs = vms.filter(v => v.vm_type === 'qemu' && v.os_type === 'windows')
  const lxcVMs     = vms.filter(v => v.vm_type === 'lxc')

  // Summary — only QEMU VMs count for alerts
  const qemuVMs       = vms.filter(v => v.vm_type === 'qemu')
  const totalUpdates  = qemuVMs.reduce((a, v) => a + (v.update_count  || 0), 0)
  const totalSecurity = qemuVMs.reduce((a, v) => a + (v.security_count || 0), 0)
  const upToDate      = qemuVMs.filter(v => !v.update_count && v.status !== 'error').length
  const withUpdates   = qemuVMs.filter(v => v.update_count > 0).length
      ...
      {checking && (
        <AlertBox type="info">
          <div className="flex items-center gap-2">
            ...
            Patch check in progress — checking Linux and Windows VMs via Guest Agent...
          </div>
        </AlertBox>
      )}

      {/* Summary — QEMU only */}
      {!loading && qemuVMs.length > 0 && (
        ...
        : vms.length === 0
          ? (
            <Card>
              <Empty icon={Package} title="No patch data"
                description={canCheck ? "Click 'Check all VMs' to scan for available updates." : "No patch data available"}
          ...
          : (
            <div className="space-y-8">
              ...
              {/* Group 3 — LXC (info only, no alarms) */}
              {lxcVMs.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Box className="w-4 h-4 text-gray-400" />
                    <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">LXC Containers</h2>
                    <span className="text-xs text-gray-400">({lxcVMs.length})</span>
                    <span className="text-xs text-gray-400 italic">— listed only, patch check not supported</span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {lxcVMs.map(vm => (
                      <VMCard key={`${vm.node}-${vm.vm_id}`} vm={vm} onSelect={setSelected} />
                    ))}
                  </div>
                </div>
              )}
