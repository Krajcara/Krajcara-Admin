          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">({data.licences.total} total · {data.licences.paid} paid · {data.licences.free} free)</span>
                </div>
              </CardHeader>
              <div className="px-5 pb-5">
                {data.licences.cost_by_currency && Object.keys(data.licences.cost_by_currency).length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Cost (incl. tax) */}
                    <div className="bg-gray-800/40 dark:bg-gray-800/60 rounded-xl border border-gray-700 p-4">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-700/50">
                          {Object.entries(data.licences.cost_by_currency).map(([cur, costs]) => (
                            <tr key={cur}>
                              <td className="py-2 font-semibold text-blue-400">{cur}</td>
                              <td className="py-2 text-right text-gray-300 font-mono">{fmtNum(costs.gross_monthly)}</td>
                              <td className="py-2 text-right text-gray-300 font-mono">{fmtNum(costs.gross_annual)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    {/* Savings */}
                    <div className="bg-green-900/20 rounded-xl border border-green-800/40 p-4">
                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Savings (free / bonus)</p>
                      {data.licences.free > 0 ? (
                        <div className="space-y-2">
                          <p className="text-green-400 font-semibold">{data.licences.free} free licence{data.licences.free > 1 ? 's' : ''}</p>
                          {data.licences.free_list?.map((l, i) => (
                            <p key={i} className="text-xs text-green-300/70">✓ {l.vendor} — {l.licence_type} ({l.licence_count} seats)</p>
                          ))}
                        </div>
                      ) : (
                        <p className="text-gray-500 text-sm">No free licences</p>
                      )}
                    </div>
                  </div>
                ) : (
                  <p className="text-sm text-gray-400 text-center py-4">No paid licences configured</p>
                )}
              </div>
            </Card>
            ...
            {data.entra?.total > 0 && (
              <Card>
                ...
                <div className="px-5 pb-5 grid grid-cols-3 gap-4">
                  <StatBox label="Total apps"       value={data.entra.total}       color="blue" />
                  <StatBox label="Expiring this month" value={data.entra.expiring_30} color={data.entra.expiring_30 > 0 ? 'red' : 'gray'} />
                  <StatBox label="OK"               value={data.entra.total - data.entra.expiring_30} color="green" />
                </div>
              </Card>
            )}
